<div x-data="{ showDeleteModal: false }">
    ...
    <div>
        <a href="{{ route('admin.categories.create') }}"
            class="inline-flex items-center px-4 py-2 bg-[#FF9D00] border border-transparent  font-semibold text-white uppercase tracking-widest hover:bg-[#8b4f17] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#FF9D00]">Add
            Category</a>
    </div>
                    ...
                    <tr>
                        ...
                        <td class="border-[var(--color-border)] border-[0.25rem] h-20 p-0">
                            <a href="{{ route('admin.categories.edit', $category) }}"
                                class="w-full h-1/2 bg-[var(--color-accent)] flex items-center justify-center text-[var(--color-bg-primary)]">
                                Edit
                            </a>
                            <hr class="border-t-[0.25rem] border-[var(--color-border)]" />
                            <button wire:click="confirmDelete({{ $category->id }})" @click="showDeleteModal = true"
                                class="w-full h-1/2 bg-[var(--color-error)] flex items-center justify-center text-[var(--color-bg-primary)]">
                                Delete
                            </button>
                        </td>
                    </tr>
        ...
        @foreach ($categories as $category)
            <div class="bg-[var(--color-bg-secondary)] p-4 r shadow space-y-3">
                <div class="flex justify-between items-center">
                    ...
                    <span class="text-xs bg-[var(--color-accent)] text-[var(--color-bg-primary)] px-2 py-1">ID:
                        {{ $category->id }}</span>
                </div>
                <div>
                    <img src="{{ asset('uploads/category/' . $category->image) }}" class="h-16 w-auto" />
                </div>
                ...
                <div class="flex w-full space-x-2 pt-2">
                    <a href="{{ route('admin.categories.edit', $category) }}"
                        class="flex-1 bg-[var(--color-accent)] text-[var(--color-bg-primary)] px-3 py-2 text-center">Edit</a>
                    <button wire:click="confirmDelete({{ $category->id }})" @click="showDeleteModal = true"
                        class="flex-1 bg-[var(--color-error)] text-[var(--color-bg-primary)] px-3 py-2">Delete</button>
                </div>
            </div>
        @endforeach
    </div>
    ...
    <div x-show="showDeleteModal" x-cloak
        ...
        <div class=" bg-[var(--color-bg-secondary)] r p-6 w-full max-w-xl">
