<div x-data="{ showDeleteModal: false }">
    ...
    <div>
        <a href="{{ route('products.create') }}" class="inline-flex items-center px-4 py-2 bg-[#FF9D00] border border-transparent  font-semibold text-white uppercase tracking-widest hover:bg-[#8b4f17] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#FF9D00]">Add Product</a>
    </div>
        ...
        @foreach ($products as $product)
            <div class="bg-[var(--color-bg-secondary)] p-4 r shadow space-y-3">
                <div class="flex justify-between items-center">
                    ...
                    <span class="text-xs bg-[var(--color-accent)] text-[var(--color-bg-primary)] px-2 py-1">{{ $product->category ? $product->category->name : 'N/A' }}</span>
                </div>
                <div>
                ...
                <div class="flex w-full space-x-2 pt-2">
                    <a href="{{ route('products.edit', $product->id) }}" class="flex-1 bg-[var(--color-accent)] text-[var(--color-bg-primary)] px-3 py-2 text-center">Edit</a>
                    <button wire:click="confirmDelete({{ $product->id }})" @click="showDeleteModal = true" class="flex-1 bg-[var(--color-error)] text-[var(--color-bg-primary)] px-3 py-2">Delete</button>
                </div>
            </div>
        @endforeach
    </div>
